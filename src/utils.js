...
export function bionify() {
  ...
  function createFloatingButton() {
    if (document.getElementById("bionify-floating-button")) return;

    var button = document.createElement("button");
    button.id = "bionify-floating-button";
    ...
    button.addEventListener("pointermove", (event) => {
      ...
      var left = Math.max(0, Math.min(window.innerWidth - button.offsetWidth, startLeft + deltaX));
      var top = Math.max(0, Math.min(window.innerHeight - button.offsetHeight, startTop + deltaY));
    ...
    style.id = "bionify-floating-style";
    style.textContent =
      "#bionify-floating-button {" +
      " position: fixed; right: 10px; top: 50%; transform: translateY(-50%);" +
      " z-index: 2147483647; width: 34px; height: 34px; padding: 0;" +
      " border: 0; border-radius: 50%; background: #222; color: #fff;" +
      " font: 700 17px sans-serif; line-height: 34px; text-align: center; cursor: grab;" +
      " box-shadow: 0 2px 8px rgba(0,0,0,.28); opacity: .88;" +
      " } #bionify-floating-button:hover { opacity: 1; }";
